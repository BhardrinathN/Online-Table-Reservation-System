...
<form id="reserveForm" action="./resvere.php" method="POST">
...
// Function to update min and max date
function updateDateLimits() {
    ...
    dateInput.setAttribute('min', formatDate(currentDate));
    // Set the max date to 10 days from today
...
// Update min and max date initially
updateDateLimits();
...
function handleDateChange() {
    ...
    if (selectedDate > maxDate) {
        // If selected date is beyond 10 days from today, reset to 10 days from today
        dateInput.value = formatDate(maxDate);
    }
    const minDate = new Date();
    if (selectedDate < minDate) {
        // If selected date is earlier than today, reset to today's date
...
// Periodically update min and max date every hour
setInterval(updateDateLimits, 3600000); // 3600000 milliseconds = 1 hour

// Periodically check for date changes and update max date
